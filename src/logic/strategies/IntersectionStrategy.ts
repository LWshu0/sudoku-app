      ...
      for (let boxCol = 0; boxCol < 3; boxCol++) {
        ...
        for (let num = 1; num <= 9; num++) {
          ...
          if (positions.length >= 2 && positions.length <= 3) {
            const isSameRow = positions.every(
              (p) => Math.floor(p / 9) === Math.floor(positions[0] / 9)
            );
            ...
            if (isSameRow) {
              ...
              // 检查该行在宫外的其他格子是否有这个数字
              const eliminations = this.findEliminationsInRow(
                row,
                num,
                positions,
                candidates,
            ...
            if (isSameCol) {
              ...
              const eliminations = this.findEliminationsInCol(
                col,
                num,
                positions,
                candidates,
